...
class ConvLayer(object):  # (28, 28, 1, 5, 5, 6, 0, 1, Activators.SigmoidActivator(),0.02)   (12, 12, 6, 5, 5, 12, 0, 1, Activators.SigmoidActivator(),0.02)
    ...
    def __init__(self, input_width, input_height, channel_number, filter_width, filter_height, filter_number,
        ...
        self.output_width = int(
            ConvLayer.calculate_output_size(self.input_width, filter_width, zero_padding, stride))  # 计算输出宽度
        self.output_height = int(
            ConvLayer.calculate_output_size(self.input_height, filter_height, zero_padding, stride))  # 计算输出高度
        self.output_array = np.zeros(
            (self.filter_number, self.output_height, self.output_width))  # 创建输出三维数组。每个过滤器都产生一个二维数组的输出
    ...
    def forward(self, input_array):  # input_array是由cnn_try过来的onepic
        ...
        self.padded_input_array = padding(input_array,
                                          self.zero_padding)  # 先将输入补足0   因为self.zero_padding为0，所以不补0，还是input_array
    ...
    def bp_gradient(self, sensitivity_array):
        ...
        for i in range(self.filter_number):  # 每个过滤器产生一个输出
            ...
            for d in range(filter.weights.shape[0]):  # 过滤器的每个通道都要计算梯度
                a[d] = conv1(self.padded_input_array[d], expanded_error_array[i], filter.weights_grad[d], 1,
                             0)  # 公式（31、32中间）
...
class MaxPoolingLayer(object):  # (24, 24, 6, 2, 2, 2)   (8, 8, 12, 2, 2, 2)
    ...
    def backward(self, input_array, sensitivity_array):
        ...
        for d in range(self.channel_number):
            for i in range(self.output_height):
                for j in range(self.output_width):
                    patch_array = get_patch(input_array[d], i, j, self.filter_width, self.filter_height,
                                            self.stride)  # 获取卷积区
...
class FullConnectedLayer(object):  # (192, 10, Activators.SigmoidActivator(),0.02)
    ...
    def backward(self, delta_array):
        ...
        def tanh_p(output):  # 前向计算，计算输出
            return 1 - output * output

        self.delta = np.multiply(tanh_p(self.input), np.dot(self.W.T, delta_array))   #计算当前层的误差，以备上一层使用
